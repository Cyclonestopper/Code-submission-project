#!/bin/bash
echo "Starting compilation of submissions/fast-fork-551 (1).cpp"
g++ -o "fast-fork-551 (1)" "submissions/fast-fork-551 (1).cpp" 2> compile_errors.log
if [ $? -ne 0 ]; then
...
for input_file in test_cases/*.in; do
    ...
    ./fast-fork-551 (1) < "$input_file" > program_output.txt
...
rm "fast-fork-551 (1)" program_output.txt compile_errors.log
